...
`save_on_epoch_end`
  - **Default**: `true`
  - **Purpose**: Also save a checkpoint (and HF model, if `hf_save_interval > 0`) at the last step of every epoch. Set to `false` when the training set is small enough that an epoch is only a few steps, otherwise every step triggers a save.
